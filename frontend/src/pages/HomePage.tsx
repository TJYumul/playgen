...
async function fetchRecommendations(audio_url: string, user_id: string): Promise<RecommendedSong[]> {
  ...
  return (
    <div className="space-y-2">
      ...
      <Slider
        ...
        max={100}
        step={1}
        onValueChange={(v: number[]) => onChange(clamp01to100(v?.[0] ?? 50))}
      />
...
export default function HomePage() {
  ...
  const [playRequest, setPlayRequest] = useState<Track | null>(null)
  ...
  const handleSongPlay = async (track: { audio_url: string }) => {
    const audioUrl = track?.audio_url
    if (typeof audioUrl !== "string" || audioUrl.trim().length === 0) return
    ...
    try {
      const results = await fetchRecommendations(audioUrl, userId)
      setRecommendedSongs(Array.isArray(results) ? results : [])
    } catch {
      setRecommendedSongs([])
    } finally {
      ...
      <SidebarInset className="bg-background">
        ...
        <main className="flex-1 overflow-y-auto p-6 pb-24">
          <div className="max-w-5xl mx-auto">
              ...
              <CardContent>
                ...
                {isGenerating ? (
                  <div className="space-y-3">
                    {Array.from({ length: 8 }).map((skeletonRow, i) => (
                      <div key={`${i}-${String(skeletonRow)}`} className="flex items-center gap-3">
            ...
            <Card className="bg-card/80 border-border backdrop-blur-sm mt-6">
              <CardHeader className="pb-4">
                    ...
                    {recommendedSongs.map((s, idx) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-muted-foreground text-xs truncate">{s.artist}</p>
                        </div>
        ...
        <MusicPlayerBar playRequest={playRequest} onSongPlay={handleSongPlay} />
      </SidebarInset>
    </SidebarProvider>
  )
}
