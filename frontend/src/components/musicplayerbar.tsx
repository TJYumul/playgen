...
type MusicPlayerProps = {
  playRequest?: Track | null
  onSongPlay?: (track: Track) => void
}
...
export default function MusicPlayer({ playRequest = null, onSongPlay }: MusicPlayerProps) {
  ...
  // Allow external UI (e.g. recommendations) to request playback.
  useEffect(() => {
    if (!playRequest) return
    if (typeof playRequest.audio_url !== "string" || playRequest.audio_url.trim().length === 0) return

    const audio = audioRef.current
    if (!audio) return

    const existingIndex = songList.findIndex((t) => t.id === playRequest.id)
    const nextIndex = existingIndex >= 0 ? existingIndex : songList.length

    if (existingIndex < 0) {
      setSongList((prev) => [...prev, playRequest])
    }

    setCurrentSongIndex(nextIndex)

    audio.src = playRequest.audio_url
    audio.load()

    void audio.play().catch((err: unknown) => {
      setIsPlaying(false)
      console.error("[player] playRequest failed", err)
    })
  }, [playRequest, songList])
  ...
  }, [currentSong?.id, currentSong?.title, logEvent, nextSong, onSongPlay, currentSong])
