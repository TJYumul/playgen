...
/**
 * Fetch full song rows for a list of song IDs.
 *
 * NOTE: Supabase does not preserve ordering when using `.in()`, so callers
 * should reorder results to match the original ID list.
 *
 * @param {string[]} songIds
 * @returns {Promise<Array<{ id: string, title: string, artist: string, audio_url: string, image_url: string }>>}
 */
export async function getSongsByIds(songIds) {
  if (!Array.isArray(songIds) || songIds.length === 0) return [];

  console.log("Incoming IDs:", songIds);

  const ids = songIds
    .map((id) => String(id).replace("track_", ""))
  ...
  const { data, error } = await supabase
    .from("songs")
    .select("id,title,artist,audio_url,image_url,jamendo_id")
    .in("id", ids);
